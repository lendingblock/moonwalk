...
async def test_basic_bitcoin_ops_fee_not_split(fee_mocker):
    bitcoin = Bitcoin()
    addr1, priv1 = await bitcoin.create_wallet()
    addr2, priv2 = await bitcoin.create_wallet()
    addr3, priv3 = await bitcoin.create_wallet()
    ...
    await bitcoin.post('sendtoaddress', addr1, 20500 / COIN)
    await bitcoin.post('generate', 1)
    assert await bitcoin.get_balance(addr1) == D(20500) / COIN
    ...
    # 500 fee subtracted from change
    assert (await bitcoin.get_balance(addr1)) == D(0)
    assert (await bitcoin.get_balance(addr2)) == D(5000) / COIN
    assert (await bitcoin.get_balance(addr3)) == D(15000) / COIN
...
async def test_insufficient_amount_to_cover_fee(fee_mocker):
    ...
    await bitcoin.post('sendtoaddress', addr1, 10000 / COIN)
    with pytest.raises(NotEnoughAmountError):
        await bitcoin.send_money(priv1, [(addr2, D(10000) / COIN)],
                                 split_fee=False)
